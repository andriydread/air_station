...
async function refreshHistory() {
  ...
  const per = data.bucket_seconds >= 60 ? `${data.bucket_seconds / 60} min` : `${data.bucket_seconds} s`;
  note.textContent = data.resolution === 'hourly'
    ? 'Hourly averages, min and max in the tooltip.'
    : `10-second rows averaged per ${per}.`;
}
